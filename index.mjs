...
const API_KEY ="live_igH6GMqmvOSSpcT52pCqLBlUly0gVQtjiX53PewaZxmGZHicBNr6ZQSxOO3hmNAf";
...
async function handleBreedSelectChange() {
    try {
      ...
      images.forEach(image => {
        const carouselItem = Carousel.createCarouselItem(image.url);
        Carousel.addItem(carouselItem);
      });
  
      // Display breed information in infoDump
      infoDump.innerHTML = `
        <h2>${images[0].breeds[0].name}</h2>
        <p>${images[0].breeds[0].description}</p>
